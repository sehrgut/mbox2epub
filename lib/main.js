...
function Mbox2Epub(opts, cb) {
	...
	var generator_props = ['uuid', 'title', 'language', 'author', 'subjects',
							'description', 'rights', 'date', 'cover'];
	var build_props = ['use_text', 'strip_background_images', 'flatten_tables'];
	...
	this.stream = function() {
		...
		for (var i=0, n=messages.length; i<n; i++) {
			var msg = messages[i];
			console.error(fmt('Found %s (%s)', msg.subject, msg.date));
